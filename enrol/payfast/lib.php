...
class enrol_payfast_plugin extends enrol_plugin {
    ...
    function enrol_page_hook(stdClass $instance) {
        ...
        if ( (float) $instance->customint1 <= 0 ) {
            $customint1 = (float) $this->get_config('customint1');
        } else {
            $customint1 = (float) $instance->customint1;
        }

        if ( (float) $instance->customint2 <= 0 ) {
            $customint2 = (float) $this->get_config('customint2');
        } else {
            $customint2 = (float) $instance->customint2;
        }
        ...
        if (abs($cost) < 0.01) { // no cost, other enrolment methods (instances) should be used
            ...
            if (isguestuser()) { // force login only for guest user, not real users with guest role
                ...
                // adjust amount for Student and Member users
                foreach (get_user_roles(context_system::instance(), $USER->id) as $role) {
                    if ($role->name == 'Member' || $role->shortname == 'member') {
                        $localisedcost = format_float($customint2, 2, true);
                        $formArray['amount'] = format_float($customint2, 2, false);
                        break;
                    }
                    if ($role->name == 'Student' || $role->shortname == 'student') {
                        $localisedcost = format_float($customint1, 2, true);
                        $formArray['amount'] = format_float($customint1, 2, false);
                        break;
                    }
                }
                ...
                if ($USER->country && $USER->country !== 'ZA') {
                    $localisedcost = format_float(round(($localisedcost / 115) * 100, 2), 2, true);
                    $formArray['amount'] = format_float(round(($formArray['amount'] / 115) * 100, 2), 2, false);
                }
