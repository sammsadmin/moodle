...
$first_time_exam_registration_model = new first_time_exam_registration_model();
...
$PAGE->set_url('/blocks/user_forms/view_forms/first_time_exam_registration.php');
$PAGE->set_title('First Time Exam Registration');
$PAGE->set_heading('First Time Exam Registration');
...
$mform = new first_time_exam_registration($form_model, $user_id, array('returnto' => $returnto));
...
if ($mform->is_submitted() && $mform->get_data()) {
    if ($form_model->check_submit($DB)) {
        ...
        //get modules for submission
        $arr_select_modules = [];
        foreach ($data as $key => $input) {
            if (strpos($key, 'first_exam_|_reg_input_identifier_course_id_') !== false) {
                $intval = intval(str_replace('first_exam_|_reg_input_identifier_course_id_', '', $key));
                if ($input == 1) {
                    $arr_select_modules[] = intval(str_replace('first_exam_|_reg_input_identifier_course_id_', '', $key));
                }
                unset($data->$key);
            }
        }
        ...
        try {
            ...
            $insert_modules_selected = false;
            $transaction->rollback($e);
        }

        if ($insert_modules_selected) {
            $first_time_exam_registration_model->insert_modules_for_submission($arr_select_modules, $form_model->current_submit_id);
            //Admin use link to see form
            $url_link = new moodle_url('/blocks/user_forms/view_forms/first_time_exam_registration.php?uid=' . $user_id);
            ...
            if (isset($arr_admin_emails) && is_array($arr_admin_emails) && count($arr_admin_emails) > 0) {
                foreach ($arr_admin_emails as $admin_email) {
                    first_time_registration_email($url_link, $user_id, $admin_email);
                }
            }

            first_time_registration_email_student($url_link, $user_id);
